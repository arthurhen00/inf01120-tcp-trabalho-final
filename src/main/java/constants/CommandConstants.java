package constants;

public final class CommandConstants {

    public static final String CHANGE_VOLUME_FUNCTION = ":CON(7";
    public static final String END_FUNCTION = ")";
    public static final String CHANGE_INSTRUMENT_FUNCTION = "I";

    public static final String INSTRUMENT_AGOGO = "!";
    public static final String INSTRUMENT_HARPSICHORD = "i";
    public static final String INSTRUMENT_TUBULAR_BELLS = "\n";
    public static final String INSTRUMENT_PAN_FLUTE = ";";
    public static final String INSTRUMENT_CHORD_ORGAN = ",";
    public static final String CHANGE_VOLUME = " ";
    public static final String CHANGE_OCTAVE = "?";
    public static final String PAUSE = "R";
}
